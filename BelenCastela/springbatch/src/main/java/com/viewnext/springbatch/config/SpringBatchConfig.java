...
import com.viewnext.springbatch.step.chunk.DistritoItemWriter;
...
public class SpringBatchConfig {
	...
	@Bean
    public DistritoItemWriter itemWriterDistrito() {
        return new DistritoItemWriter();
    }
	...
	public Step readFile(JobRepository jobRepository, PlatformTransactionManager transactionManager) throws ParseException {
		return new StepBuilder("readFile", jobRepository)
				.<Direccion, Direccion>chunk(4000, transactionManager)
	...
	@Bean
    public Step countDistritosStep(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
        return new StepBuilder("countDistritosStep", jobRepository)
                .<Direccion, Direccion>chunk(2000, transactionManager)
                .reader(itemReaderDireccion())
                .processor(itemProcessorCounterDistrito())
                .writer(itemWriterDistrito())
                .taskExecutor(taskExecutor())
                .faultTolerant()
                .skipPolicy(new AlwaysSkipItemSkipPolicy())
                .skip(FlatFileParseException.class)
	...
	public Job job(JobRepository jobRepository, @Qualifier("readFile") Step readFile, @Qualifier("countDistritosStep") Step countDistritosStep) {
		return new JobBuilder("firstBatchJob", jobRepository)
				.start(readFile)
				.next(countDistritosStep)
				.build();
	}
